...
define(['cbj', 'kenedo', 'configbox/server'], function(cbj, kenedo, server) {
	...
	kenedo.registerSubviewReadyFunction('view-admincalcformula', function() {

		kenedo.addStylesheet(server.config.urlSystemAssets + '/css/calc-editor.css?' + server.config.cacheVar);
		...
		// Create a jQuery function for making an HTML element draggable the way we want it
		...
		// Store button (overrides the calculation form task button)
		cbj('.task-apply, .task-copy, .task-store').click(CalcEditor.storeCalculation);
		...
		cbj('#element-filter').keyup(function(){
	...
	var CalcEditor = {
		...
		/**
		 * Generates the calculation JSON and fill the calculation form field '#calc'
		 */
		storeCalculation: function() {
			...
			var calcJson = JSON.stringify(calcObjects);

			// Fill the input field for storing
			cbj('#calc').val(calcJson);
		...
		removeSelectedItems : function() {
			...
			cbj('#view-admincalcformula #terms .parameter').each(function(i,parameter){

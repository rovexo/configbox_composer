...
class JFormFieldListing extends JFormField {
	...
	protected function getInput() {
		...
		$link = KLink::getRoute('index.php?option=com_configbox&controller=adminlistings&tmpl=component&parampicker=1&pickerobject='.$pickerObject);
		...
		if ($value) {
			$item->title = ConfigboxCacheHelper::getTranslation('#__configbox_strings', 20, $value, $tag);
		} 
		else {
			$listings = KenedoModel::getModel('ConfigboxModelAdminListings')->getRecords();
			if (count($listings)) {
				$item->title = $listings[0]->title;
				$value = $listings[0]->id;
			}
		}
		...
		<div id="<?php echo $pickerObject;?>-modal" tabindex="-1" class="modal hide fade jviewport-width80">
			...
			<div class="modal-footer">
				<button type="button" class="btn" data-dismiss="modal" aria-hidden="true"><?php KText::_('Close');?></button>
			</div>
		</div>

...
a("#row_calc_id").val(b).trigger("chosen:updated").trigger("change"),a(".trigger-add-row").show());"column"===c?(a(".cell-column-parameter").find(".axis-label").hide(),a(".cell-column-parameter").find(".label-calculation").show().find(".parameter-title").text(h)):(a(".cell-row-parameter").find(".axis-label").hide(),a(".cell-row-parameter").find(".label-calculation").show().find(".parameter-title").text(h));var d='<i class="dragtable-drag-handle row-sort-handle fa fa-reorder"></i>';d+='<input class="input-value" type="text" value="" />';
d+='<span class="trigger-remove"></span>';a("row"===c?".row-parameter":".column-parameter").each(function(){0===a(this).find("input[type=text]").length&&a(this).html(d)});a(this).closest(".kenedo-popup").hide()},onQuestionPicked:function(){var b=a(this).val(),h=a(this);e.makeRequest("ajaxapi","getAnswerDropdownData",{question_id:b}).done(function(c){var d=0===h.closest(".column-parameter-picker").length?"row":"column";"column"===d?(a("#column_type").val("question"),a("#column_calc_id").val("0").trigger("chosen:updated").trigger("change"),
a("#column_element_id").val(b).trigger("chosen:updated").trigger("change"),a(".trigger-add-column").show()):(a("#row_type").val("question"),a("#row_calc_id").val("0").trigger("chosen:updated").trigger("change"),a("#row_element_id").val(b).trigger("chosen:updated").trigger("change"),a(".trigger-add-row").show());var f=h.find("option[value="+b+"]").text(),e="";"column"===d?(a(".cell-column-parameter").find(".axis-label").hide(),e+='<i class="dragtable-drag-handle column-sort-handle fa fa-reorder"></i>'):
(a(".cell-row-parameter").find(".axis-label").hide(),e+='<i class="dragtable-drag-handle row-sort-handle fa fa-reorder"></i>');var g=0;a.each(c,function(){g++});1<g?("column"===d?(a(".cell-column-parameter").find(".axis-label").hide(),a(".cell-column-parameter").find(".label-answers").show().find(".parameter-title").text(f)):(a(".cell-row-parameter").find(".axis-label").hide(),a(".cell-row-parameter").find(".label-answers").show().find(".parameter-title").text(f)),e+="<select>",a.each(c,function(a,
b){e+='<option value="'+a+'">'+b+"</option>"}),e+="</select>"):("column"===d?(a(".cell-column-parameter").find(".axis-label").hide(),a(".cell-column-parameter").find(".label-textfield").show().find(".parameter-title").text(f)):(a(".cell-row-parameter").find(".axis-label").hide(),a(".cell-row-parameter").find(".label-textfield").show().find(".parameter-title").text(f)),e+='<input class="input-value" type="text" value="" />');e+='<span class="trigger-remove"></span>';var l=1;a("row"===d?".row-parameter":
".column-parameter").each(function(){1<g?a(this).html(e):0===a(this).find("input[type=text]").length&&a(this).html(e);0!==a(this).find("select").length&&(a(this).find("select option:nth-child("+l+")").prop("selected",!0),l++)});h.closest(".kenedo-popup").hide()})},addRow:function(){a(".calc-matrix tr:last").clone().appendTo(".calc-matrix");a(".calc-matrix tr:last .input-value").val("");a(".calc-matrix tr:last .price").val("")},addColumn:function(){a(".calc-matrix thead tr:first-child").find("th:last").clone().appendTo(a(".calc-matrix thead tr:first-child")).find("input").val("");

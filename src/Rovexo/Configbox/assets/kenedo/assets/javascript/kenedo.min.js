...
".kenedo-new-tab, .new-tab",function(a){window.open(this.href);a.preventDefault()});a(document).on("click",".property-type-translatable .language-switcher",function(){a(this).siblings().removeClass("active");a(this).addClass("active");var b=a(this).attr("for");a("#translation-"+b).show().siblings().hide()});a(document).on("change",".kenedo-property :input",function(){var b=a(this).closest(".kenedo-property").attr("id").replace("property-name-",""),c=a(this).val(),e=a(this).closest(".kenedo-details-form").data("record");
"undefined"!=typeof e[b]&&(e[b]=c);a(this).closest(".kenedo-details-form").trigger("kenedoFormDataChanged",[e,b,c])});a(document).on("click",".property-group-using-toggles>.property-group-legend",function(){a(this).closest(".property-group").hasClass("property-group-opened")?(a(this).closest(".property-group").removeClass("property-group-opened").addClass("property-group-closed"),a(this).closest(".property-group").find(".property-group-toggle-state").val("closed")):(a(this).closest(".property-group").removeClass("property-group-closed").addClass("property-group-opened"),
a(this).closest(".property-group").find(".property-group-toggle-state").val("opened"))});a(document).on("change",".join-select",function(){a(this).closest(".property-body").find(".join-link").hide();a(this).closest(".property-body").find(".join-link-"+a(this).val()).show()});a(document).on("click",".show-file-uploader",function(b){b.preventDefault();b=a(this).closest(".property-body").find(".file-upload-field").clone();a(this).data("inputfield",b);a(this).closest(".property-body").find(".file-uploader").slideDown(300);
a(this).closest(".property-body").find(".file-current-file, .file-delete, .file-upload").slideUp(300)});a(document).on("click",".file-upload-cancel",function(b){b.preventDefault();a(this).closest(".property-body").find(".file-uploader").slideUp(300);a(this).closest(".property-body").find(".file-current-file, .file-delete, .file-upload").slideDown(300);b=a(this).closest(".property-body").find(".show-file-uploader").data("inputfield");a(this).closest(".property-body").find(".file-upload-field").replaceWith(b)});
a(document).on("click",".order-property",d.changeListingOrder);a(document).on("change",".listing-filter",d.changeListingFilters);a(document).on("click",".link-save-item-ordering",d.storeOrdering);a(document).on("click",".kenedo-check-all-items",d.toggleCheckboxes);a(document).on("click",".kenedo-pagination-list a",d.changeListingStart);a(document).on("change",".kenedo-limit-select",d.changeListingLimit);a(document).on("click",".kenedo-trigger-toggle-active",d.toggleActive);a(document).on("click",
".kenedo-details-form .kenedo-task-list .task",d.executeFormTask);a(document).on("click",".kenedo-listing-form .kenedo-task-list .task",d.executeListingTask);a(document).on("click",".listing-link",d.openListingLink);a(document).on("keyup",".ordering-text-field",function(){a(this).closest(".kenedo-listing").find(".link-save-item-ordering").show()});a(document).on("mouseenter",".kenedo-popup-trigger",f.cancelClosing);a(document).on("mouseenter",".kenedo-popup-trigger",f.scheduleOpening);a(document).on("mouseleave",
".kenedo-popup-trigger",f.scheduleClosing);a(document).on("mouseleave",".kenedo-popup-trigger",f.cancelOpening);a(document).on("mouseleave",".kenedo-popup",f.scheduleClosing);a(document).on("mouseenter",".kenedo-popup",f.cancelClosing);a(".kenedo-popup-trigger").click(function(b){b.stopPropagation();b.preventDefault();a(this).trigger("mouseenter")});a(document).on("click",".trigger-select-all-multi",function(){a(this).closest(".kenedo-property").find(".property-body option").prop("selected",!0);a(this).closest(".kenedo-property").find(".property-body input").prop("checked",
!0)});a(document).on("click",".trigger-deselect-all-multi",function(){a(this).closest(".kenedo-property").find(".property-body option").prop("selected",!1);a(this).closest(".kenedo-property").find(".property-body input").prop("checked",!1)})},loadSubview:function(b,c,e,h){c||(c=".configbox-ajax-target");var k;if("object"==typeof b){if(a(this).closest(".intra-listing").length)return;k=b.value}"string"==typeof b&&""!=b&&(k=b);if(!k)throw"loadSubview called, but no URL could be determined.";!1!==h&&
window.history.pushState({isSubview:!0,url:k,targetSelector:c,callbackFunction:e},"",k);-1==k.indexOf("ajax_sub_view")&&(k="magento"==u.config.platformName?k+"ajax_sub_view/1/format/raw":k+(-1==k.indexOf("?")?"?ajax_sub_view=1&format=raw":"&ajax_sub_view=1&format=raw"));cbrequire(["tinyMCE"],function(a){3<a.majorVersion&&a.remove()});a(c).animate({opacity:".0"},200,function(){a(c).load(k+" .kenedo-view:first",function(b,h,k){200!==k.status&&(b='<div id="view-error" class="kenedo-view kenedo-ajax-sub-view"><div class="kenedo-listing-form">'+
("<p>Encountered a system error: HTTP code is "+k.status+". Text message is: "+k.statusText+"</p>"),b+="</div>",b+="</div>",a(c).html(b));d.initNewContent(a(c));a(c).animate({opacity:"1"},400,function(){d.afterInitNewContent(a(c));"function"==typeof e&&e(a(c))})})})},initNewContent:function(b){0!=a(b).length&&(a(b).find("select.listing-filter, select.join-select, .property-type-dropdown select, .make-me-chosen, .chosen-dropdown").length&&cbrequire(["cbj.chosen"],function(){a(b).find("select.listing-filter, select.join-select, .property-type-dropdown select, .make-me-chosen, .chosen-dropdown").chosen({disable_search_threshold:10,
search_contains:!0,inherit_select_classes:!0,width:"100%"});a(b).find(".chosen-container:hidden").each(function(){a(this).is(".join-select")?a(this).css("width","100%"):a(this).closest(".kenedo-properties").length?a(this).css("width","100%"):a(this).css("width","auto")})}),a(b).find(".sortable-listing tbody").length&&cbrequire(["cbj.ui"],function(){a(b).find(".sortable-listing tbody").sortable(d.listingSortableSettings)}),a(b).find(".datepicker").length&&cbrequire(["cbj.ui"],function(){a(b).find(".datepicker").datepicker({dateFormat:"yy-mm-dd"})}),
0!==a(b).find(".kenedo-datepicker").length&&cbrequire(["cbj.ui"],function(){a(b).find(".kenedo-datepicker").each(function(){var c=a(this).closest(".kenedo-property").find(".form-control"),e=c.val(),c={dateFormat:"yy-mm-dd",altFormat:"yy-mm-dd",altField:c};a(b).find(".kenedo-datepicker").datepicker(c);e&&"0000-00-00"!==e&&a(b).find(".kenedo-datepicker").datepicker("setDate",e)})}),a(b).find(".kenedo-view").each(function(){var b=a(this).attr("id");b&&d.runSubviewReadyFunctions(b)}),a(b).find(".kenedo-details-form").each(function(){var b=
a(this).closest(".kenedo-details-form").data("record");d.setPropertyVisibility(null,b);a(this).on("kenedoFormDataChanged",d.setPropertyVisibility)}),a(b).find(".kenedo-listing .ordering-text-field").attr("autocomplete","off"),a(document).trigger("cbViewInjected"))},afterInitNewContent:function(a){0!==a.find(".kenedo-html-editor").length&&cbrequire(["tinyMCE","configbox/server"],function(a,b){try{a.init({convert_urls:!1,document_base_url:b.config.urlBase,documentBaseURL:b.config.urlBase,baseURL:b.config.urlTinyMceBase,
suffix:!0===b.config.useMinifiedJs?".min":"",mode:"textareas",selector:".kenedo-html-editor",plugins:["advlist autolink link image lists charmap print preview hr anchor pagebreak spellchecker","searchreplace wordcount visualblocks visualchars code fullscreen insertdatetime nonbreaking","save table directionality emoticons template paste"],toolbar:"insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | l      ink image | print preview media fullpage | forecolor backcolor emoticons",

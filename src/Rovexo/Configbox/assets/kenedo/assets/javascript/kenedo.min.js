...
define(["cbj","configbox/server"],function(c,l){var f={initAdminPageEach:function(a){a.find(".kenedo-listing-form:not(.default-handlers-attached)").each(function(){var b=c(this);b.find(".trigger-kenedo-form-task").each(function(){c(this).on("click",f.onListTaskButtonClicked)});b.on("cbListTaskTriggered",f.onListTaskTriggered);b.on("cbListTaskResponseReceived",f.onListTaskResponseReceived);b.addClass("default-handlers-attached");b.trigger("cbDefaultHandlersAttached")});a.find(".kenedo-details-form:not(.default-handlers-attached)").each(function(){var b=
...
a))},onListTaskTriggered:function(a,b){switch(b.task){case "add":if(!1===0<b.list.closest(".intra-listing").length)a=f.getListParameter(b.list,"add-link"),a=f.base64UrlDecode(a),f.loadSubview(a);else{a=b.list.closest(".kenedo-details-form");var e=f.getListParameter(b.list,"controller"),d=f.getListParameter(b.list,"foreignKeyField"),h=f.getListParameter(b.list,"foreignKeyPresetValue");f.openDetailsFormModal(b.list,a,e,"edit",0,d,h)}break;case "copy":case "remove":case "delete":a=f.getListParameter(b.list,
"controller");e={ids:f.getCheckedListItemIds(b.list).join(",")};l.makeRequest(a,b.task,e).then(function(g,m,r){b.list.trigger("cbListTaskResponseReceived",[r,b])});break;default:console.log('Unknown task "'+b.task+'"')}},onListTaskResponseReceived:function(a,b,e){e.btn.removeClass("disabled");try{var d=JSON.parse(b.responseText)}catch(g){console.warn('Could not parse JSON response. Response text was: "'+b.responseText+'"');console.warn("Error message follows:");console.warn(g);return}switch(e.task){case "copy":case "remove":var h=
e.list.data("view");f.refreshList(e.list,function(){e.list=c(".kenedo-view[data-view-id="+h+"]").find(".kenedo-listing-form");f.showResponseMessages(e.list,d.errors||[],d.messages||[])})}c(document).trigger("cbListTaskResponseReceivedGlobal",[b,e])},registerSubviewReadyFunction:function(a,b){"undefined"==typeof f.subviewReadyFunctions[a]&&(f.subviewReadyFunctions[a]=[]);f.subviewReadyFunctions[a].push(b)},runSubviewReadyFunctions:function(a){"undefined"!=typeof f.subviewReadyFunctions[a]&&c.each(f.subviewReadyFunctions[a],
function(b,e){"function"==typeof e&&e()})},toggleCheckboxes:function(){var a=c(".kenedo-listing .kenedo-check-all-items").prop("checked");c(this).closest(".kenedo-listing").find(".kenedo-item-checkbox").prop("checked",a)}},k={openingDelay:200,closingDelay:200,openers:[],closers:[],clearOpeners:function(){c.each(k.openers,function(a,b){"undefined"!=typeof b&&window.clearTimeout(b.timeout)});k.openers=[]},clearClosers:function(){c.each(k.closers,function(a,b){"undefined"!=typeof b&&window.clearTimeout(b.timeout)});
k.closers=[]},scheduleOpening:function(){var a=c(this).attr("id").replace("kenedo-popup-trigger-","");k.openers[a]&&window.clearTimeout(k.openers[a].timeout);k.openers[a]={popupId:a,timeout:window.setTimeout(function(){k.open(a)},k.openingDelay)}},scheduleClosing:function(){var a;c(this).is(".kenedo-popup")&&(a=c(this).attr("id").replace("kenedo-popup-",""));c(this).is(".kenedo-popup-trigger")&&(a=c(this).attr("id").replace("kenedo-popup-trigger-",""));k.closers[a]&&window.clearTimeout(k.closers[a].timeout);
k.closers[a]={popupId:a,timeout:window.setTimeout(function(){k.close(a)},k.closingDelay)}},cancelOpening:function(){var a;c(this).is(".kenedo-popup")&&(a=c(this).attr("id").replace("kenedo-popup-",""));c(this).is(".kenedo-popup-trigger")&&(a=c(this).attr("id").replace("kenedo-popup-trigger-",""));k.openers[a]&&window.clearTimeout(k.openers[a].timeout)},cancelClosing:function(){var a;c(this).is(".kenedo-popup")&&(a=c(this).attr("id").replace("kenedo-popup-",""));c(this).is(".kenedo-popup-trigger")&&
(a=c(this).attr("id").replace("kenedo-popup-trigger-",""));k.closers[a]&&window.clearTimeout(k.closers[a].timeout)},open:function(a){var b=c("#kenedo-popup-original-"+a).clone(!0);c("#kenedo-popup-"+a).remove();b.hide();b.attr("id","kenedo-popup-"+a);b.appendTo("body");k.position(a);b.fadeIn();c("#kenedo-popup-trigger-"+a).trigger("popup-open")},close:function(a){c("#kenedo-popup-"+a).fadeOut(200,function(){c(this).remove()});c("#kenedo-popup-trigger-"+a).trigger("popup-close")},position:function(a){var b=
c("#kenedo-popup-trigger-"+a);a=c("#kenedo-popup-"+a);a.css("visibility","hidden");a.css("display","block");c("body").append(a);var e=b.find(".kenedo-popup-trigger-content");b=e.outerWidth();var d=e.outerHeight(),h=e.offset(),g=a.find(".kenedo-popup-content").outerWidth(),m=a.find(".kenedo-popup-content").outerHeight(),r=Math.sqrt(1352)/2;a.outerWidth(g);a.outerHeight(m+r);var n=Math.max(document.documentElement.clientWidth,window.innerWidth||0);e=Math.max(document.documentElement.clientHeight,window.innerHeight||
0);var q=c("body").scrollTop(),u=c("body").scrollLeft(),t=h.left+b/2-g/2;b=h.top-m-r;d=h.top+d+r;m=d+m;0>t+u?(u=10+u,t-=u,h=document.createElement("style"),document.head.appendChild(h),h.sheet.insertRule(".kenedo-popup-content:before {left:"+t+"px;}",h.sheet.cssRules.length),h.sheet.insertRule(".kenedo-popup-content:after {left:"+t+"px;}",h.sheet.cssRules.length),t=u):(u=document.createElement("style"),document.head.appendChild(u),u.sheet.insertRule(".kenedo-popup-content:before {left:auto;}",u.sheet.cssRules.length),
u.sheet.insertRule(".kenedo-popup-content:after {left:auto;}",u.sheet.cssRules.length));g+t>n&&(u=n-g-10,t-=u,g=document.createElement("style"),document.head.appendChild(g),g.sheet.insertRule(".kenedo-popup-content:before {left:"+t+"px;}",g.sheet.cssRules.length),g.sheet.insertRule(".kenedo-popup-content:after {left:"+t+"px;}",g.sheet.cssRules.length),t=u);g=b>q;e=m<e+q;q=null;a.hasClass("position-prefer-top")&&g&&(q=!0);a.hasClass("position-prefer-bottom")&&e&&(q=!1);null===q&&(q=g);q?(a.addClass("position-top").removeClass("position-bottom"),
a.offset({left:t,top:b})):(a.addClass("position-bottom").removeClass("position-top"),a.offset({left:t,top:d}));a.css("display","none");a.css("visibility","visible");return a}},p={_keyStr:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",encode:function(a){var b="",e=0;for(a=p._utf8_encode(a);e<a.length;){var d=a.charCodeAt(e++);var h=a.charCodeAt(e++);var g=a.charCodeAt(e++);var m=d>>2;d=(d&3)<<4|h>>4;var r=(h&15)<<2|g>>6;var n=g&63;isNaN(h)?r=n=64:isNaN(g)&&(n=64);b=b+this._keyStr.charAt(m)+
this._keyStr.charAt(d)+this._keyStr.charAt(r)+this._keyStr.charAt(n)}return b},decode:function(a){var b="",e=0;for(a=a.replace(/[^A-Za-z0-9\+\/=]/g,"");e<a.length;){var d=this._keyStr.indexOf(a.charAt(e++));var h=this._keyStr.indexOf(a.charAt(e++));var g=this._keyStr.indexOf(a.charAt(e++));var m=this._keyStr.indexOf(a.charAt(e++));d=d<<2|h>>4;h=(h&15)<<4|g>>2;var r=(g&3)<<6|m;b+=String.fromCharCode(d);64!=g&&(b+=String.fromCharCode(h));64!=m&&(b+=String.fromCharCode(r))}return b=p._utf8_decode(b)},
_utf8_encode:function(a){a=a.replace(/\r\n/g,"\n");for(var b="",e=0;e<a.length;e++){var d=a.charCodeAt(e);128>d?b+=String.fromCharCode(d):(127<d&&2048>d?b+=String.fromCharCode(d>>6|192):(b+=String.fromCharCode(d>>12|224),b+=String.fromCharCode(d>>6&63|128)),b+=String.fromCharCode(d&63|128))}return b},_utf8_decode:function(a){for(var b="",e=0,d,h,g;e<a.length;)d=a.charCodeAt(e),128>d?(b+=String.fromCharCode(d),e++):191<d&&224>d?(h=a.charCodeAt(e+1),b+=String.fromCharCode((d&31)<<6|h&63),e+=2):(h=a.charCodeAt(e+
1),g=a.charCodeAt(e+2),b+=String.fromCharCode((d&15)<<12|(h&63)<<6|g&63),e+=3);return b}};return f});
